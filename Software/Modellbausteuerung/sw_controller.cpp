...
void Controller_class::add_toggle_switch(int channel, int pin, int power, bool inverted, bool memory, int lower_limit, int upper_limit, int time)
{
	if (counter_ < NUM_SWITCHES) {
		channel_[counter_] = channel;
		switches_[counter_] = new Toggle_switch_class(pin, power, inverted, memory, lower_limit, upper_limit, time);
		counter_++;
	}
}
